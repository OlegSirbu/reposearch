...
export const useRepositorySearch = (): UseRepositorySearchProps => {
  ...
  const { data, loading, error, fetchMore } = useQuery<QueryResult>(GET_REPOSITORIES, {
    variables: { query, first: PAGE_SIZE, after: endCursor },
  });

  const updateQuery = useCallback((newQuery: string, newEndCursor: string | null, newCurrentPage: number) => {
    setQuery(newQuery);
  ...
  }, [updateQuery, fetchMore]);

  const handlePageChange = useCallback((page: number) => {
    const newEndCursor = (page !== 1 && data?.search?.pageInfo.endCursor) || null;
    updateQuery(query, newEndCursor, page);

      newEndCursor && fetchMore({
        variables: {
          query,
          first: PAGE_SIZE,
          after: newEndCursor,
